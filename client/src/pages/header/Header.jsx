...
import { updateSearchValue } from "@/redux/searchSlice";
...
const Header = () => {
  ...
  const handleSearch = (e) => {
    ...
    setInterval(() => {
      dispatch(updateSearchValue(searchItem));
    }, 2000);
  };
    ...
    <div className="bg-[#FFFFFF] text-sm pt-[20px] pl-[100px] pr-[100px]">
            ...
            <ul className="flex gap-[10px] list-none">
              {isLoggedIn ? (
                <div className="flex gap-[10px]">
                  ...
                  <Link className=" hover:text-hover-color" to={"/create"}>
                    {" "}
                    Create{" "}
                  </Link>
                </div>
      ...
      <div className="flex justify-between items-center pb-[32px]">
        ...
        <div className="flex items-center text-[18px]">
          <Link to={"/startfree"}><h1 className="text-dark-color hover:text-blue-600 hover:underline">Get started free</h1></Link>
        </div>
      </div>
    </div>
  );
